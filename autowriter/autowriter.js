...
var Autowriter = (function() {
    var $notebook = $('#notebook'),
        nb = document.getElementById('notebook');
    return {
        init: function() {
            var saved = localStorage.getItem("saved-notebook");
            if (saved) $notebook.val(saved);

            $notebook.click(function(ev){
                //$notebook.textrange('setcursor', $notebook.val().length);
            }).keydown(function(ev){
                if (ev.which == 8) return Autowriter.backspace();
                //$notebook.textrange('setcursor', $notebook.val().length);
            }).keyup(function(){
            ...
            $(document).keydown(function(ev){
                ...
                range = $notebook.textrange('get');
                text = $notebook.val();
                if (range.start < text.length) {
                    $notebook.textrange('setcursor', text.length);
                }
            });

            $('#clear').click(Autowriter.clear);

            window.setTimeout(Autowriter.autosave, 5000);
        },
        backspace: function() {
            var range = $notebook.textrange('get'),
                text = $notebook.val();
            // nothing selected

            if (range.start < text.length) return false;
            if (range.start - range.end == 0) {
                if (" \r\t\n\v".indexOf(text.charAt(range.start - 1)) > -1) {
                    return false;
                } else {
                    return true;
                }
        ...
        autosave: function() {
            ...
            localStorage.setItem("saved-notebook", $notebook.val());
            window.setTimeout(Autowriter.autosave, 5000);
        }
    };
})();
...
if (!Modernizr.localstorage) {
    alert("This site requires HTML5 localstorage");
} else {
    Autowriter.init();
}
